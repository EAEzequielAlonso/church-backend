  ...
  async execute(
    ...
    if (!dto.ministryId && !dto.categoryId) {
      throw new BadRequestException(
        'At least one of Ministry or Category must be specified',
      );
    }
    ...
    if (existingAllocation) {
      throw new BadRequestException(
        'Ya existe una asignación presupuestaria para esta combinación de Ministerio y Categoría en este periodo.',
      );
    }
    ...
    try {
      ...
      if (error.code === '23505') {
        ...
        throw new BadRequestException(
          'Budget Allocation for this Ministry/Category already exists in this Period',
        );
      }
      throw error;
    }
