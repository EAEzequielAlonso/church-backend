...
export class GetBudgetExecutionUseCase {
  ...
  async execute(churchId: string, periodId: string) {
    ...
    const rawStats = await this.dataSource
    ...
    const results = allocations.map((allocation) => {
      ...
      const matchingStats = rawStats.filter((stat) => {
        if (allocation.ministry && stat.ministryId !== allocation.ministry.id)
          return false;
        if (allocation.category && stat.categoryId !== allocation.category.id)
          return false;
        return true;
      });

      const spentAmount = matchingStats.reduce(
        (sum, stat) => sum + Number(stat.totalParams),
        0,
      );
    ...
    const totalExecuted = rawStats.reduce(
      (sum, stat) => sum + Number(stat.totalParams),
      0,
    );
