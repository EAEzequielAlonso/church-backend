...
import { BudgetPeriod, BudgetPeriodType } from '../entities/budget-period.entity';
...
export class CreateBudgetPeriodUseCase {
    ...
    async execute(dto: CreateBudgetPeriodDto, churchId: string): Promise<BudgetPeriod> {
        return this.dataSource.transaction(async (manager) => {
            ...
            const overlap = await periodRepo.createQueryBuilder('bp')
            ...
            if (overlap) {
                throw new BadRequestException(`Budget Period overlaps with existing period: ${overlap.name}`);
            }
