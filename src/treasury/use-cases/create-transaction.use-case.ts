...
export class CreateTransactionUseCase {
  ...
  async execute(
    data: CreateTransactionDto & { userId: string; churchId: string },
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (data.sourceAccountId) {
        sourceAccount = await accountRepo.findOne({
          where: { id: data.sourceAccountId, church: { id: data.churchId } },
        });
        if (!sourceAccount)
          throw new NotFoundException(
            'Source Account not found or does not belong to this church.',
          );
      }

      if (data.destinationAccountId) {
        destinationAccount = await accountRepo.findOne({
          where: {
        ...
        if (!destinationAccount)
          throw new NotFoundException(
            'Destination Account not found or does not belong to this church.',
          );
      }
