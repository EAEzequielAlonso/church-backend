    ...
    try {
      ...
      if (tx.status === TransactionStatus.COMPLETED) {
        if (tx.sourceAccount && tx.sourceAccount.type === AccountType.ASSET) {
          tx.sourceAccount.balance =
            Number(tx.sourceAccount.balance) + Number(tx.amount);
          await accountRepo.save(tx.sourceAccount);
        }

        if (
          tx.destinationAccount &&
          tx.destinationAccount.type === AccountType.ASSET
        ) {
          const destAmount = Number(tx.amount) * Number(tx.exchangeRate);
          tx.destinationAccount.balance =
            Number(tx.destinationAccount.balance) - destAmount;
      ...
      if (err.code === '23503') {
        ...
        throw new BadRequestException(
          'No se puede eliminar la transacción porque está siendo utilizada en otro registro.',
        );
      }

      throw new BadRequestException(
        `Error al eliminar (debug): ${err.message || 'Error desconocido'}`,
      );
    } finally {
