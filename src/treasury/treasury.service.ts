...
export class TreasuryService {
  ...
  constructor(
    ...
    private auditRepo: Repository<TreasuryAuditLog>,
  ...
  async findAllCategories(churchId: string, type?: string) {
    ...
    if (type)
      where.type =
        type === 'income' ? TransactionType.INCOME : TransactionType.EXPENSE; // Basic mapping
  ...
  async createAccount(data: any, churchId: string) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Invalid Account Type. Must be ASSET, LIABILITY, or EQUITY.',
      );
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (type === TransactionType.INCOME) {
        ...
        if (!destinationAccountId)
          throw new BadRequestException(
            'INCOME requires a Destination Account (where money goes).',
          );
        if (!categoryId)
          throw new BadRequestException('INCOME requires a Category.');
        if (sourceAccountId)
          throw new BadRequestException(
            'INCOME source must be null (use Category).',
          );

        const dest = await manager.findOne(Account, {
          where: { id: destinationAccountId },
        });
        const cat = await manager.findOne(TransactionCategory, {
          where: { id: categoryId },
        });
        ...
        if (!sourceAccountId)
          throw new BadRequestException(
            'EXPENSE requires a Source Account (where money comes from).',
          );
        if (!categoryId)
          throw new BadRequestException('EXPENSE requires a Category.');
        if (destinationAccountId)
          throw new BadRequestException(
            'EXPENSE destination must be null (use Category).',
          );
      ...
      if (ministryId) {
        const ministry = await manager.findOne(Ministry, {
          where: { id: ministryId },
        });
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (tx.type === TransactionType.INCOME && tx.destinationAccount) {
        const acc = await manager.findOne(Account, {
          where: { id: tx.destinationAccount.id },
        });
        if (acc) {
        ...
        const src = await manager.findOne(Account, {
          where: { id: tx.sourceAccount.id },
        });
        const dst = await manager.findOne(Account, {
          where: { id: tx.destinationAccount.id },
        });
        if (src) {
          src.balance = Number(src.balance) + Number(tx.amount);
          await manager.save(src);
        }
        if (dst) {
          dst.balance = Number(dst.balance) - Number(tx.amount);
          await manager.save(dst);
        }
      }
  ...
  async getBudgets(churchId: string, year?: number) {
    return this.budgetRepo.find({
      where: { church: { id: churchId }, ...(year ? { year } : {}) },
