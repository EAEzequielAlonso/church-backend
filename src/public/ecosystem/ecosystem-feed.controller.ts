...
export class EcosystemFeedController {
  constructor(
    private readonly feedService: EcosystemContributionsService,
  ) { }
  ...
  async getGlobalFeed(
    ...
    const events = await this.feedService.getFeed(limit, offset, undefined, churchId);
  ...
  async getPersonalFeed(
    ...
    const events = await this.feedService.getFeed(limit, offset, req.user.id);
    return events.map(this.mapToFeedItem);
  }
