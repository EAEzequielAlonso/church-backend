...
import {
  ChurchMapItemDto,
  MissionMapItemDto,
  SmallGroupMapItemDto,
  NeedSignalMapItemDto,
  ChurchNeedSignalMapItemDto,
  UnreachedAreaMapItemDto,
} from './dto/map-item.dto';
...
export class GeoService {
  ...
  private readonly cacheTtlMs =
    Number(process.env.GEO_CACHE_TTL ?? 86400) * 1000;
  ...
  private cacheKey(input: GeocodeInput) {
    return JSON.stringify({
      a: this.normalizeText(input.address).toLowerCase(),
      c: this.normalizeText(input.city).toLowerCase(),
      s: this.normalizeText(input.state).toLowerCase(),
      o: this.normalizeText(input.country).toLowerCase(),
    });
  }
  ...
  async geocodeChurchAddress(input: GeocodeInput) {
    const normalizedInput = {
      ...
      country: this.normalizeText(input.country),
    };
    ...
    try {
      response = await fetch(
        `${base}/search?format=jsonv2&limit=1&addressdetails=1&q=${encodeURIComponent(q)}`,
        {
          signal: controller.signal,
          headers: {
            'User-Agent': 'Saas-Iglesia/1.0 (public geocoding service)',
          },
        },
      );
    } catch {
      throw new BadGatewayException(
        'No pudimos conectar con el proveedor de geocoding.',
      );
    } finally {
    ...
    if (response.status === 429)
      throw new HttpException(
        'Límite temporal de geocoding alcanzado. Intenta de nuevo en unos segundos.',
        429,
      );
    if (!response.ok)
      throw new BadGatewayException(
        'El proveedor de geocoding no respondió correctamente.',
      );
    ...
    if (!first)
      throw new BadGatewayException(
        'No encontramos una ubicación para esa dirección.',
      );
    ...
    const payload = {
      ...
      country: this.normalizeText(
        first.address?.country ?? normalizedInput.country,
      ),
    };
  ...
  async getViewport(query: ViewportQueryDto) {
    ...
    const [churches, missions, groups, needs, churchNeeds, unreached] =
      await Promise.all([
        this.dataSource.query(
          `
        SELECT p.id, c.name, p.latitude, p.longitude, p.city, p.state, p."isCurrentAdmin", p."isVerified" as verified
      ...
      missions: missions.map(
        (m: any) =>
          ({
            ...
            collaborationCount: Number(m.collaborationCount),
            needCount: Number(m.needCount),
            type: 'MISSION',
          }) as MissionMapItemDto,
      ),
      smallGroups: groups.map(
        (g: any) =>
          ({
            ...g,
            ...
            type: 'SMALL_GROUP',
          }) as SmallGroupMapItemDto,
      ),
      needSignals: needs.map(
        (n: any) =>
          ({
            ...n,
            latitude: Number(n.latitude),
            longitude: Number(n.longitude),
            supportCount: Number(n.supportCount),
            type: 'NEED_SIGNAL',
          }) as NeedSignalMapItemDto,
      ),
      churchNeedSignals: churchNeeds.map(
        (cn: any) =>
          ({
            ...cn,
            latitude: Number(cn.latitude),
            longitude: Number(cn.longitude),
            type: 'CHURCH_NEED_SIGNAL',
          }) as ChurchNeedSignalMapItemDto,
      ),
      unreachedAreas: unreached.map(
        (u: any) =>
          ({
            ...u,
