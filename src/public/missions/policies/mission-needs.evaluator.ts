...
import { MissionNeedAction, MissionProjectStatus, MissionNeedStatus } from '../enums/missions.enums';
import { MissionProject } from '../entities/mission-project.entity';
import { MissionNeed } from '../entities/mission-need.entity';
import { MISSION_ALLOWS_NEED_FULFILLMENT_STATES, MISSION_EDITABLE_STATES } from './mission.constants';
...
export class MissionNeedsEvaluator {
  getAllowedActions(
    ...
    if (
      ...
      (need.status === MissionNeedStatus.OPEN || need.status === MissionNeedStatus.IN_PROGRESS)
    ) {
