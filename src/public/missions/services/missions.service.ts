...
export class MissionsService {
  ...
  async findAllManagementByChurch(
    churchId: string,
    query: PaginationQueryDto,
    actor: Person,
  ): Promise<PaginatedResponseDto<MissionProductDto>> {
    const { page = 1, limit = 12, sort = 'createdAt', order = 'DESC' } = query;
    ...
    const items = data.map((mission) => {
      const allowedActions = this.missionRules.getProjectAllowedActions(
        mission,
        true,
      );
      ...
      return MissionProductDto.fromResponse(
        responseDto,
        allowedActions,
        emptyStats,
      );
  ...
  async changeStatus(
    ...
    // Eventos unívocos
    if (
      previousStatus === MissionProjectStatus.DRAFT &&
      newStatus === MissionProjectStatus.ACTIVE
    ) {
      await this.activitiesService.logActivity({
        actorPersonId: actor.id,
        activityType: EcosystemActivityType.MISSION_CREATED,
      ...
      previousStatus === MissionProjectStatus.ACTIVE &&
      newStatus === MissionProjectStatus.PAUSED
    ) {
      await this.activitiesService.logActivity({
        actorPersonId: actor.id,
      ...
      previousStatus === MissionProjectStatus.PAUSED &&
      newStatus === MissionProjectStatus.ACTIVE
    ) {
      await this.activitiesService.logActivity({
        actorPersonId: actor.id,
  ...
  async completeMission(
    ...
    const mission = await this.findOne(id);
    await this.missionRules.assertCanManage(actor, mission, isChurchAdmin);
    this.missionRules.assertCanChangeState(
      mission.status,
  ...
  async cancelMission(
    ...
    const saved = await this.missionsRepo.manager.transaction(
      async (manager) => {
        ...
        await manager
          ...
          .andWhere('status != :withdrawnStatus', {
            withdrawnStatus: MissionCollaborationStatus.WITHDRAWN,
          })
          .execute();
