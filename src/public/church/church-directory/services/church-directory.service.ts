...
export class ChurchDirectoryService {
  constructor(
    @InjectRepository(ChurchPublicProfile) private readonly repo: Repository<ChurchPublicProfile>,
  ) { }

  async find(query: ChurchDirectoryQueryDto) {
    const qb = this.repo.createQueryBuilder('p').innerJoinAndSelect('p.church', 'c');
    if (query.city) qb.andWhere('LOWER(p.city) = LOWER(:city)', { city: query.city });
    if (query.state) qb.andWhere('LOWER(p.state) = LOWER(:state)', { state: query.state });
    if (query.country) qb.andWhere('LOWER(p.country) = LOWER(:country)', { country: query.country });
    if (query.verified !== undefined) qb.andWhere('p.isVerified = :verified', { verified: query.verified === 'true' });
    if (query.search) qb.andWhere('(LOWER(c.name) LIKE :q OR LOWER(p.city) LIKE :q)', { q: `%${query.search.toLowerCase()}%` });
    ...
    if (query.sort === 'city') qb.orderBy('p.city', 'ASC').addOrderBy('c.canonicalName', 'ASC');
    else if (query.sort === 'newest_claimed') qb.orderBy('p.updatedAt', 'DESC');
    else qb.orderBy('p.isVerified', 'DESC').addOrderBy('c.canonicalName', 'ASC');
    ...
    return {
      data: rows.map((r) => ({
        ...
        location: { latitude: r.latitude ? Number(r.latitude) : null, longitude: r.longitude ? Number(r.longitude) : null },
